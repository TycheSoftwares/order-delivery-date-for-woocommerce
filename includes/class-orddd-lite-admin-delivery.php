...
class Orddd_Lite_Admin_Delivery {
	...
	public static function orddd_lite_meta_box( $order, $post ) {
		...
		if ( 'on' === get_option( 'orddd_enable_delivery_date' ) ) {
			...
			$date_field_label        = __( get_option( 'orddd_lite_delivery_date_field_label' ), 'order-delivery-date' );
			$time_field_label        = __( get_option( 'orddd_delivery_timeslot_field_label' ), 'order-delivery-date' );
            ...
            if ( isset( $data['_orddd_lite_timestamp'][0] ) && '' !== $data['_orddd_lite_timestamp'][0] ) {
				$default_date           = date( 'd-m-Y', $data['_orddd_lite_timestamp'][0] );
				$default_h_deliverydate = date( 'j-n-Y', $data['_orddd_lite_timestamp'][0] );
			...
			if ( 'on' === get_option( 'orddd_lite_enable_time_slot' ) ) {
				...
				print( '<tr id="save_delivery_date_button">
                    <td><input type="button" value="' . __( 'Update', 'order-delivery-date' ) . '" id="save_delivery_date" class="button button-primary"></td>
                    <td><input type="button" value="' . __( 'Update & Notify Customer', 'order-delivery-date' ) . '" id="save_delivery_date_and_notify" class="button button-primary"' . $disabled . '></td>
			...
			print( '
				...
				<input type="hidden" id="original_post_status" value="' . $disabled . '" />
				<input type="hidden" id="orddd_lite_default_date" name="orddd_lite_default_date" value="' . $default_date . '" />
				<input type="hidden" id="orddd_lite_minimumOrderDays" name="orddd_lite_minimumOrderDays" value="' . $default_date . '" />
				<input type="hidden" id="orddd_lite_current_day" name="orddd_lite_current_day" value="' . $current_date. '" />
				<input type="hidden" id="orddd_lite_current_date_set" name="orddd_lite_current_date_set" value="' . $current_date . '" />
				<input type="hidden" id="orddd_lite_default_h_date" name="orddd_lite_default_h_date" value="' . $default_h_deliverydate . '" />
				<input type="hidden" id="orddd_lite_holidays" name="orddd_lite_holidays" value="' . esc_attr( $holidays_str ) . '" />
				<input type="hidden" id="orddd_lite_delivery_date_format" name="orddd_lite_delivery_date_format" value="' . $delivery_date_format . '" />
				<input type="hidden" id="orddd_lite_first_day_of_week" name="orddd_lite_first_day_of_week" value="' . $first_day_of_week . '" />
				<input type="hidden" name="orddd_lite_enable_time_slot" id="orddd_lite_enable_time_slot" value="' . get_option( 'orddd_lite_enable_time_slot' ) . '">
				<input type="hidden" name="orddd_lite_date_field_mandatory" id="orddd_lite_date_field_mandatory" value="' . get_option( 'orddd_lite_date_field_mandatory' ) . '">
				<input type="hidden" name="orddd_lite_time_slot_mandatory" id="orddd_lite_time_slot_mandatory" value="' . get_option( 'orddd_lite_time_slot_mandatory' ) . '">
				<input type="hidden" name="orddd_lite_admin_url" id="orddd_lite_admin_url" value="' . get_admin_url() . '">
				<input type="hidden" name="orddd_lite_order_id" id="orddd_lite_order_id" value="' . $order_id . '">
				<input type="hidden" name="orddd_lite_min_date_set" id="orddd_lite_min_date_set" value="' . $min_date_array['min_date'] . '">
				<input type="hidden" name="orddd_lite_field_label" id="orddd_lite_field_label" value="' . $date_field_label . '">
			');
	...
	public static function save_delivery_dates() {
		...
		if ( isset( $order_id ) && $order_id > 0 && false !== get_post_status( $order_id ) ) {
			...
			if ( ( isset( $_POST['e_deliverydate'] ) && $_POST['e_deliverydate'] != '' ) ) {
				...
				if ( isset( $data['_orddd_lite_timestamp'][0] ) && '' !== $data['_orddd_lite_timestamp'][0] ) {
					$previous_order_h_date        = date( 'j-n-Y', $data['_orddd_lite_timestamp'][0] );
					$previous_order_date_check    = date( 'n-j-Y', $data['_orddd_lite_timestamp'][0] );
					$previous_order_weekday_check = date( 'w', $data['_orddd_lite_timestamp'][0] );
				}
			...
			if ( isset( $_POST['orddd_time_slot'] ) && '' !== $_POST['orddd_time_slot'] && 'Select a time slot' !== $_POST['orddd_time_slot'] && 'No time slots are available.' !== $_POST['orddd_time_slot'] ) {
				...
				$timeslot_selected = 'yes';
					  ...
					  ( 'Select a time slot' === $_POST['orddd_time_slot'] ||
					  'No time slots are available.' === $_POST['orddd_time_slot'] ) ) {
				$timeslot_selected = 'no';
			} else {
				$timeslot_selected = 'yes';
			}
			...
			if ( 'yes' === $delivery_details_updated && isset( $_POST['orddd_notify_customer'] ) &&
				'yes' === $_POST['orddd_notify_customer'] ) {
